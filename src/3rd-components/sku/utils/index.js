...
export function flatten(tree, stocks = [], options) {
  ...
  tree.forEach(sku => {
    const leaf = sku[optionValuesKey]
  ...
  for (let i = 0; i < skuLen; i++) {
    ...
    tree.forEach((sku, column) => {
      ...
      if (!sku[optionIdKey] || !item[optionIdKey]) return
      mapKey.push(`${sku[optionIdKey]}_${item[optionIdKey]}`)
      skus.push({
        k_id: sku[optionIdKey],
        k: sku[optionNameKey],
        v_id: item[optionIdKey],
        v: item[optionNameKey]
      })
    })
...
export function isEqual(prevSKU, nextSKU, options) {
  let { optionIdKey = 'uid' } = options || {}
  return (
    nextSKU.length === prevSKU.length &&
    nextSKU.every(({ leaf = [] }, index) => {
      let prevLeaf = prevSKU[index].leaf || []
      return (
        prevSKU[index][optionIdKey] === nextSKU[index][optionIdKey] &&
        leaf.length === prevLeaf.length &&
        leaf.map(item => item[optionIdKey]).join(',') ===
          prevLeaf.map(item => item[optionIdKey]).join(',')
      )
    })
  )
}
